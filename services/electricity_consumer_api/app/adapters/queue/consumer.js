...
const consumer = new Consumer(
    client,
    [
        { topic: 'Group2-Power-Logs', partition: 0 }
        ],
    {
...
exports.consumerOn = async () => {
    ...
    consumer.on('message',async (message) => {
        ...
        await pg_client.query(`insert into log_electricity_consumption(school_id,class_id,sensor_id,sensor_data,read_at) 
            ...
            .then((result)=>{
                rd_client.set(`group2_power_${result.rows[0].id}`,JSON.stringify(result.rows[0])).then((message)=>{
                    console.log("keys: ",message)
                }).catch((err)=>{
                    const errData = {
                        flag_type: 1,
                        req_src: "consumer-power-api",
                        req_path: "/",
