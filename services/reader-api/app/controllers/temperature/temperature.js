...
const temperatureGet = async (req, res) => {
  ...
  .then(async (redis_res) => {
    ...
    if (rd_token.token != null && rd_token.token == refreshToken) {
      await rd_client.LRANGE("temperature-group2", -1,(-1)*lastEl)  // key, start, end = ..., -1, -10 => last 10 elements
      .then((res_rd)=>{
    ...
    const errData = {
      ...
      req_src: "reader-api",
      req_path: "/",
      req_file: "temperature.js",
      req_line: 26,
      req_func: "temperatureGet",
      req_type: "Controller",
      req_raw: req.body,
      content_err: err,
